package net.leo.mgmod.enchantment.effects;
...
public record LightningEnchantmentEffect(EnchantmentLevelBasedValue amount) implements EnchantmentEntityEffect {
    ...
    public void apply(ServerWorld world, int level, EnchantmentEffectContext context, Entity target, Vec3d pos) {
        if(target instanceof LivingEntity entity && context.owner() instanceof PlayerEntity player) {
            float number_of_strikes = this.amount.getValue(level);

            BlockPos target_position = entity.getBlockPos();
            for(int i =0; i < number_of_strikes; i++) {
                EntityType.LIGHTNING_BOLT.spawn(world, target_position, SpawnReason.TRIGGERED);
            }
        }
    }
